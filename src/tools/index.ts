...
export const addTimelineCommentTool: Tool = {
  name: 'bitrix24_add_timeline_comment',
  description: 'Post a free-form comment/note to a Bitrix24 CRM entity timeline (deal, contact, lead, company). Supports BBCode formatting.',
  inputSchema: {
    type: 'object',
    properties: {
      entityId: { type: 'number', description: 'ID of the CRM entity' },
      entityType: { type: 'string', enum: ['deal', 'contact', 'lead', 'company'], description: 'CRM entity type' },
      comment: { type: 'string', description: 'Comment text. BBCode supported: [B]bold[/B], [URL=..]text[/URL], [LIST][*]item[/LIST]' }
    },
  ...
  try {
    switch (name) {
      ...
      case 'bitrix24_create_activity': {
        ...
        const activityResult = await bitrix24Client.createActivity(fields);
      ...
      case 'bitrix24_update_activity': {
        ...
        await bitrix24Client.updateActivity(args.id, updateFields);
